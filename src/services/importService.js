...
let getLanKiemKe = () => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let data = await db.sequelize.query('CALL sp_get_lankiemke',
                {
                    // replacements: { id: 2 },
                    raw: false
                }
...
let getAllNguoiSuDung = () => {
    return new Promise(async (resolve, reject) => {
        try {

            const request = db.sqlhis2Connection.request();
            const getdata = await request.execute('[sp_QLPO_GETALLNGUOISUDUNG]');
            // Lấy dữ liệu từ recordset
            const data = getdata.recordset;
            //console.log('recordset', data)
            resolve(data)
...
let getCheckDongBo = (LanKiemKe, NamKiemKe) => {
    return new Promise(async (resolve, reject) => {
        try {

            const request = db.sqlhis2Connection.request();
            request.queryTimeout = 60000;
            const getdata = await request.execute('[sp_QLPO_CHECK_DONGBO]');
            const datataisan = getdata.recordset;
            // const time = new Date();
            // const NamKiemKe = time.getFullYear();
            //console.log('data', datataisan)

            let datatool = await db.sequelize.query('CALL sp_getdatatoll(:LanKiemKe,:NamKiemKe)',
                {
                    replacements: {
                        LanKiemKe: LanKiemKe,
                        NamKiemKe: NamKiemKe
            ...
            const plainDatatool = JSON.parse(JSON.stringify(datatool));
            //console.log('datataisan', datataisan)
            //console.log('plainDatatool', plainDatatool)

            const result = datataisan.filter(dts =>
                !plainDatatool.some(pdt =>
                    pdt.KhoDuoc_Id === dts.KhoDuoc_Id &&
                    pdt.KhoaPhongSuDung === dts.KhoaPhongSuDung &&
                    pdt.KhoaQuanLy === dts.KhoaQuanLy
                )
            );
            ...
            result.forEach(dts => {
                ...
                const lanKiemKe = plainDatatool[0].LanKiemKe;
                // Thêm đối tượng vào plainDatatool với DaDongBo = 0
                plainDatatool.push({
                    ...dts,        // Sao chép các thuộc tính từ datataisan
                    LanKiemKe: lanKiemKe, // Giữ lại LanKiemKe
        ...
        try {
            if (KhoQuanLy === null && KhoTaiSan_Id !== null) {
                ...
                if (data.length === 0) {
                    ...
                    // if (LanKiemKe === null) {
                    //     console.log('LanKiemKe 1 ', LanKiemKe)
                    // } else {
                    //     console.log('LanKiemKe 2', LanKiemKe)
                    // }
                    ...
                    if (datakiemke.length === 0) {
                        if (LanKiemKe >= 1) {
                            ...
                            let getDotKiemKe = await db.sequelize.query('CALL sp_get_dot_kiemke(:KhoTaiSan_Id,:KhoQuanLy,:LanKiemKe,:NamKiemKe)',
                                {
                                    replacements: {
                                        KhoTaiSan_Id: KhoTaiSan_Id,
                                        KhoQuanLy: KhoQuanLy,
                            ...
                            if (DotKiemKe_Id_Int) {
                                ...
                                let getduocdakiem = await db.sequelize.query('CALL sp_get_duocda_kiemke_theonam(:KhoQuanLy,:LanKiemKe,:DotKiemKe_Id_Int,:NamKiemKe)',
                                    {
                                        replacements: {
                                            KhoQuanLy: KhoQuanLy,
                                            DotKiemKe_Id_Int: DotKiemKe_Id_Int,
                                ...
                                let getdata = await db.sequelize.query('CALL sp_get_data_kiemke_khongcodotkiemke(:KhoTaiSan_Id,:KhoQuanLy,:DotKiemKe_Id_Int,:NamKiemKe)',
                                    {
                                        replacements: {
                                            KhoTaiSan_Id: KhoTaiSan_Id,
                                            KhoQuanLy: KhoQuanLy,
                                ...
                                if (ketqua.length > 0) {
                                    ...
                                    let getdatanew = await db.sequelize.query('CALL sp_get_data_kiemke_khongcodotkiemke(:KhoTaiSan_Id,:KhoQuanLy,:DotKiemKe_Id_Int,:NamKiemKe)',
                                        {
                                            replacements: {
                                                KhoTaiSan_Id: KhoTaiSan_Id,
                                                KhoQuanLy: KhoQuanLy,
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (data.length === 1) {
                ...
                let checkdakiem_khackhoa = await db.sequelize.query('CALL sp_check_ma_dakiem_khackhoa(:MaTaiSan,:LanKiemKe,:KhoQuanLy,:Duoc_Id,:NamKiemKe)',
                    {
                        replacements: {
                            MaTaiSan: MaTaiSan,
                    ...
                    for (const item of data) {
                        try {
                            ...
                            if (data) {
                                ...
                                data.SoLuongThucTe = item.SoLuong;
                                data.ChenhLech = item.SoLuong - item.SoLuong;
                ...
                let checkdakiem_theokhoa = await db.sequelize.query('CALL sp_check_ma_dakiem_theokhoa(:MaTaiSan,:KhoTaiSan_Id,:KhoQuanLy,:DotKiemKe_Id,:NamKiemKe)',
                    {
                        replacements: {
                            MaTaiSan: MaTaiSan,
                            KhoTaiSan_Id: KhoTaiSan_Id,
                ...
                //console.log('checkdakiem_theokhoa.length', checkdakiem_theokhoa.length)
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (data.length > 0) {
                ...
                //console.log('checklankiem', checklankiem, KhoTaiSan_Id)
                //console.log('checkdakiem_khackhoa', MaTaiSan, LanKiemKe == null ? 1 : LanKiemKe + 1, KhoQuanLy)
                let checkdakiem_khackhoa = await db.sequelize.query('CALL sp_check_serial_dakiem_khackhoa(:Serial,:LanKiemKe,:KhoQuanLy,:NamKiemKe)',
                    {
                        replacements: {
                            Serial: Serial,
                            LanKiemKe: LanKiemKe == null ? 1 : LanKiemKe,
                ...
                let checkdakiem_theokhoa = await db.sequelize.query('CALL sp_check_serial_dakiem_theokhoa(:Serial,:KhoTaiSan_Id,:KhoQuanLy,:DotKiemKe_Id,:NamKiemKe)',
                    {
                        replacements: {
                            Serial: Serial,
                            KhoTaiSan_Id: KhoTaiSan_Id,
                ...
                if (dataArray.length > 0) {
                    ...
                    for (const item of dataArray) {
                        try {
                            ...
                            let data = await db.Data_KiemKe.findOne({ where: { id: item.rowId }, raw: false });
                            if (data) {
                                data.NguoiSuDung = item.NguoiSuDung;
                                data.NguoiSuDung_Id = item.NguoiSuDung_Id;
                                // Lưu thay đổi
                                await data.save();
                            }
                        } catch (error) {
